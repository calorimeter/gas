...
{ 
  ...
  fNoLayersCmd = new G4UIcmdWithAnInteger("/testem/setAmountOfLayers",this);
  fNoLayersCmd->SetGuidance("Set thickness of the absorber material");
  fNoLayersCmd->SetParameterName("N",false,false);
  fNoLayersCmd->SetRange("N>0");
  fNoLayersCmd->AvailableForStates(G4State_PreInit,G4State_Idle);

  fAbsThickCmd = new G4UIcmdWithADoubleAndUnit("/testem/setAbsorberThick",this);
  fAbsThickCmd->SetGuidance("Set thickness of the absorber material");
  fAbsThickCmd->SetParameterName("delta",false,false);
  fAbsThickCmd->SetUnitCategory("Length");
  fAbsThickCmd->SetDefaultUnit("mm");
  fAbsThickCmd->SetRange("delta>=0.");
...
{ 
  ...
  else if( command == fNoLayersCmd )
  	{
	  fDetector->SetNumberOfLayers(fNoLayersCmd->GetNewIntValue(newValue));
  	}
