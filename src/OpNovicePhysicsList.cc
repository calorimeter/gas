...
OpNovicePhysicsList::OpNovicePhysicsList() 
 : G4VUserPhysicsList(),
   fVerboseLebel(1), fMaxNumPhotonStep(30)
{
...
/*  cerenkovProcess->SetVerboseLevel(fVerboseLebel);
  ...
  while( (*theParticleIterator)() ){
    ...
    G4ProcessManager* pmanager = particle->GetProcessManager();
    G4String particleName = particle->GetParticleName();
    if(particleName == "e-" || particleName == "e+" || particleName == "pi-" || particleName == "pi+" || particleName == "p"){
        //fForwardXrayTR = new G4ForwardXrayTR("Xenon","Mylar","XrayTR") ;
        //pmanager->AddProcess(new G4VTransitionRadiation,-1,-1,2);
/*    	DetectorConstruction* p = (DetectorConstruction*) G4RunManager::GetRunManager()->GetUserDetectorConstruction();
    	fForwardXrayTR = new G4ForwardXrayTR(p->GetContainerMaterial()->GetName(),
    										 p->GetGasMaterial()->GetName(),
    										 "TR");*/
    	pmanager->AddDiscreteProcess(fForwardXrayTR);
    	//fForwardXrayTR->DumpInfo();
    	//std::cout << p->GetContainerMaterial()->GetName() << " " << p->GetGasMaterial()->GetName() << std::endl;
    }
...
void OpNovicePhysicsList::ConstructTR(G4ProcessManager* pmanager)
{
	DetectorConstruction* p = (DetectorConstruction*) G4RunManager::GetRunManager()->GetUserDetectorConstruction();
	...
	pmanager->AddDiscreteProcess(
			...
			//                                   regular radiator
			(p->GetLogicalRadiator(), // XTR radiator

					p->GetContainerMaterial(), // real foil
					p->GetGasMaterial(),  // real gas
					p->GetContainerThickness(),    // real geometry
					p->GetGasThickness(),
					p->GetNumberOfLayers(),
					"RegularXTRadiator"));
	...
	pmanager->AddDiscreteProcess(
			...
			( p->GetLogicalRadiator(),
					1000.,
					100.,
					p->GetContainerMaterial(),
					p->GetGasMaterial(),
					p->GetContainerThickness(),
					p->GetGasThickness(),
					p->GetNumberOfLayers(),
					"GammaXTRadiator"));
}
...
void OpNovicePhysicsList::SetCuts()
{
  ...
  //
  SetCutsWithDefault();
  G4ProductionCutsTable::GetProductionCutsTable()->SetEnergyRange(200*CLHEP::eV, 1000*CLHEP::GeV);
