...
/*  fAnalysisManager->FillH1(1,fTotEdep/keV,1.0);
  ...
  fAnalysisManager->FillNtupleDColumn(0, fTotEdep/keV);
  fAnalysisManager->FillNtupleDColumn(1, fCluster);

  fAnalysisManager->FillNtupleIColumn(2,fGammaCounter);

	if(fGammaCounter > 0){
		fAnalysisManager->FillNtupleDColumn(3,fGammaEnergy/CLHEP::keV);
		fAnalysisManager->FillNtupleDColumn(4,fAngle/fGammaCounter/CLHEP::cm);
	}else {
		fAnalysisManager->FillNtupleDColumn(3,0);
		fAnalysisManager->FillNtupleDColumn(4,0);
	}
...
void Run::AddEnergy(G4double edep, const G4Step* step)
{
  ...
  fTotEdep += edep;
  if(step && edep > 0) {
	  //G4cout << "Got " << edep/eV << " from " << fElIonPair->SampleNumberOfIonsAlongStep(step) << G4endl;
    ...
    if(step->GetTrack()->GetDefinition() == G4Gamma::GammaDefinition()){
    	  ...
    	  G4cout << "GammaHit " << edep/keV  << " " << step->GetDeltaEnergy()
