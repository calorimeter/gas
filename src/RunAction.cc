...
RunAction::RunAction()
  : G4UserRunAction(), fAnalysisManager(0), fRun(0), fHistName("gasresult")
{}
...
/*  TestParameters* param = TestParameters::GetPointer();
  ...
  fAnalysisManager->CreateH1("h1","empty",
                             1,0,1);
...
//  fAnalysisManager->CreateH1("h3","Amount of photons in event",
//                              50,0,50);

  fAnalysisManager->CreateNtuple("result", "Deposited Gas Energy");
  fAnalysisManager->CreateNtupleDColumn("edep");
  fAnalysisManager->CreateNtupleDColumn("nClusters");
  fAnalysisManager->CreateNtupleIColumn("nPhotons");
  fAnalysisManager->CreateNtupleDColumn("photon_energy");
  fAnalysisManager->CreateNtupleDColumn("photon_r");
...
void RunAction::BeginOfRunAction(const G4Run* aRun)
{
  ...
  OpNovicePhysicsList* pl = (OpNovicePhysicsList*) G4RunManager::GetRunManager()->GetUserPhysicsList();
  G4ForwardXrayTR* tr = pl->fForwardXrayTR;
  tr->BuildXrayTRtables();
  fRun->BeginOfRun();
